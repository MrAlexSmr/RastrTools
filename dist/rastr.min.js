...
if(!force){m.copy(addr,m.layers_get_extra(),0);}},resizeAll:(lyrs,offsetX,offsetY,width,height)=>{var i,lyr,oldW,oldH,extra,oldSize;oldW=m.layers_get_width();oldH=m.layers_get_height();if(oldW===width&&oldH===height)return false;m.layers_resize(width>>>0,height>>>0);if(Array.isArray(lyrs)&&lyrs.length<=lyrsCnt){i=lyr=0>>>0;extra=m.layers_get_extra();oldSize=(oldW*oldH)<<2;while(i<lyrs.length){lyr=lyrs[i++]>>>0;m.copy_in_range(extra,extra+oldSize,lyr);m.clear_in_range(lyr,lyr+oldSize,0);m.put_image(lyr,extra,offsetX|0,offsetY|0,oldW>>>0,oldH>>>0);}}
